...
def chat_with_claude(user_text: str, user: dict, history: list) -> tuple[str, list]:
    ...
    system = (
        ...
        "Инструмент find_task ищет по названию и описанию задачи.\n"
        "ВАЖНО: голосовой ввод часто даёт русскую транскрипцию иностранных слов или слипшиеся слова:\n"
        "  • «фортранзит» = Ford Transit → ищи «Ford», потом «Transit»\n"
        "  • «сузуки» = Suzuki, «тойота» = Toyota, «фолксваген» = Volkswagen\n"
        "  • «авансчёт» = «аванс счёт», «замерзайка» = незамерзающая жидкость\n"
        "Стратегия поиска:\n"
        "1. Сначала вызови find_task с полным запросом пользователя\n"
        "2. Если не нашёл — вызови find_task с каждым значимым словом по отдельности\n"
        "3. Если слово похоже на транслитерацию — попробуй оригинальное английское написание\n"
        "4. Если упоминается имя исполнителя — ищи задачи с этим именем отдельным запросом\n"
        "Делай несколько вызовов find_task подряд, не сдавайся после первой попытки.\n\n"
        "## База знаний\n"
        ...
        "После инструментов — краткий ответ.\n"
        "При создании или изменении задачи — ВСЕГДА заканчивай ответ строкой:\n"
        "🔗 [Открыть задачу](URL) — где URL из результата инструмента (https://a96a08a.platrum.ru/tasks?taskId=ID).\n"
        "Без ссылки ответ считается неполным."
