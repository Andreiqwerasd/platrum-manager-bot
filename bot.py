...
def chat_with_claude(user_text: str, user: dict, history: list) -> tuple[str, list]:
    ...
    system = (
        ...
        "Голосовой ввод часто искажает слова: транслитерирует английское, сливает слова, меняет окончания.\n"
        "Используй своё знание языков — если слово не найдено буквально, подумай как оно могло звучать:\n"
        "  • Русское звучание → оригинальное слово (марка, модель, термин, название)\n"
        "  • Слипшиеся слова → раздели их\n"
        "  • Сокращение → полное название\n"
        "Стратегия:\n"
        "1. find_task с исходным запросом\n"
        "2. Если пусто — подумай что имел в виду пользователь, вызови find_task с другим вариантом\n"
        "3. Повторяй до 3-4 попыток с разными вариантами написания — не сдавайся сразу\n"
        "4. Если упоминается человек — ищи задачи с его именем отдельным запросом\n\n"
        "## База знаний\n"
